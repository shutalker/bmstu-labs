...
double GetAcceleration(const std::vector<NodeStat> &stats, int subspaceCount,
  ...
  int nodesPerSubspace = ceil((double)(nodesToCompute) / subspaceCount);
  int maxSubspacesPerProcessor = ceil((double)(subspaceCount) / stats.size());
  int nodesToComputePerProcessor = maxSubspacesPerProcessor * nodesPerSubspace;

  double solutionTimeParallel = (maxSubspacesPerProcessor + 1) * T_S \
      + (DIM + M) * nodesToComputePerProcessor * L * networkDiameter * T_C \
      + T * nodesToComputePerProcessor * computationComplexity;
        ...
        for (const auto &r: N_REPEATS) {
          ...
          if (r == 1) {
            ...
            for (int iRepeat = 0; iRepeat < r; ++iRepeat) {
              accelerations[iRepeat] = GetAcceleration(stats, nSubspaces,
                  computationComlexityDist(randomGenerator));
            }
